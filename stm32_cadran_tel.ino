...
HardwareSerial Serial2(PA3,PA2);  // port série relié au module BT201


void setup(){
  //start serial connections
  Serial.begin(9600);  // monitor for debug
  Serial2.begin(115200); // link to the BT201 module
...
void loop(){
  ...
  case 0:
    ...
    if (recucart != "") Serial.println("recu BT201 "+ recucart);
    ...
    if((recucart == "") && !decroch ) // on vient de decrocher
      {
        ...
        Serial.println("decrochage pour numerotation");
      }
    else
      {
        ...
        if((recucart != "") && decroch)
        {
          ...
          {
            etat = 1;
            Serial.println(" appel entrant " + recucart); //
          }
        }
        else
        {
          /* caracteres recus non appel entrant*/
          if (recucart != "")  Serial.println("recu? " + recucart);
          etat = 0;
        }
  ...
  case 1:
    ...
    if(!decroch) // on decroche
    {
      ...
      Serial.println("communication acceptee");
      envoicart = envoidonn("AT+BA04\r\n");
      Serial.println("recu module "+envoicart);
      etat = 4;
    }
    break;
  ...
  case 2: // on numerote apres avoir decroche
    while(noncompo)  // tant que l on a pas atteint la tempo
    {
      ...
      if (numcompo != "") 
        {
          etat = 3;
          Serial.println("Numero compose " + numcompo);
        }
      else 
        { 
          etat = 5;  //on arrete en attendant le raccroche
          Serial.println(" pas de numero compose");
        }

    break;

  case 3: //
    Serial.println(" attente etablissement comm");
  ...
  case 4: // en cours de communication
    ...
    {
      Serial.println("on coupe la communication");
  ...
  case 5: // en fin de tempo de numerotation
    Serial.println("fin tempo, attente raccrochage");
    if(decroch) etat =0;
    break;
  }
}
